...
export default function DiscoverTicker() {
    return (
        <div className="w-full bg-white border-b border-slate-100 flex items-center h-11 overflow-hidden relative">
            {/* Side Fades */}
            ...
            >
                {[...TICKER_DATA, ...TICKER_DATA].map((item, index) => (
                    <div key={index} className="flex items-center gap-3 group cursor-pointer hover:bg-slate-50 px-4 py-1.5 rounded-full transition-all duration-300">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">{item.name}</span>
                        <div className="flex items-center gap-2">
                            <span className="text-[11px] font-black text-slate-900">{item.value}</span>
                            <span className={`flex items-center gap-1 text-[10px] font-black px-1.5 py-0.5 rounded-md ${item.isUp ? "text-emerald-500 bg-emerald-50" : "text-rose-500 bg-rose-50"
                                }`}>
