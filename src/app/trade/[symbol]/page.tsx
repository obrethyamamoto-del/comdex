...
export default function TradePage({ params }: { params: Promise<{ symbol: string }> }) {
    ...
    let quoteSymbol = "AUSD";

    // Simple heuristic for pair parsing
    if (baseSymbol.endsWith("AUSD") && baseSymbol !== "AUSD") {
        baseSymbol = baseSymbol.slice(0, -4);
        quoteSymbol = "AUSD";
    } else if (baseSymbol.endsWith("USDT") && baseSymbol !== "USDT") {
        baseSymbol = baseSymbol.slice(0, -4);
        quoteSymbol = "USDT";
    }

    const asset = ASSETS.find(a => a.symbol === baseSymbol) || ASSETS[0];
    const [tradeType, setTradeType] = useState<"buy" | "sell">("buy");
    const [mounted, setMounted] = useState(false);
    const [activeTab, setActiveTab] = useState("1D");
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);

    // Initialize selected asset as the base asset of the pair
    const [selectedAsset, setSelectedAsset] = useState(asset);

    // Quote asset (the 'other' side of the trade)
    const quoteAsset = ASSETS.find(a => a.symbol === quoteSymbol) || ASSETS.find(a => a.symbol === "AUSD") || ASSETS[0];
                ...
                <div className="max-w-[1400px] mx-auto w-full px-4 md:px-10 py-6 md:py-12">
                    ...
                    <div className="flex items-center justify-between mb-8">
                        ...
                        >
                            ...
                            <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest group-hover:text-slate-900 transition-colors">Back to Discover</span>
                        </Link>
                    </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col min-w-0">
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <h1 className="text-xl md:text-2xl font-black text-slate-950 tracking-tight truncate">{selectedAsset.name} / {quoteSymbol}</h1>
                                                <span className="text-slate-400 font-bold text-base md:text-lg">Trade Pair</span>
                                            </div>
                            ...
                            <div className="bg-white rounded-[32px] p-6 md:p-8 border border-slate-100 shadow-xl lg:sticky lg:top-24">
                                            ...
                                            {tradeType === "buy" ? (
                                                <div className="flex items-center gap-2.5 px-3 py-2 bg-white border border-slate-100 rounded-2xl shadow-sm">
                                                    <div className="flex items-center gap-2">
                                                        <AssetIcon symbol={quoteAsset.symbol} avatar={quoteAsset.avatar} color={quoteAsset.color} size="sm" />
                                                        <span className="text-[14px] font-black text-slate-900 tracking-tight">{quoteAsset.symbol}</span>
                                                    </div>
                                                </div>
                                            ) : (
                                                    ...
                                                    {isDropdownOpen && (
                                                        <>
                                                            ...
                                                            <div className="absolute top-full right-0 mt-2 w-56 bg-white border border-slate-100 rounded-2xl shadow-2xl py-2 z-50 max-h-64 overflow-y-auto no-scrollbar">
                                                                {ASSETS.filter(a => a.symbol !== quoteAsset.symbol).map((a) => (
                                                                    <button
                                                                        key={a.symbol}
                                ...
                                <div className="space-y-4 pt-10 pb-8 px-1">
                                    ...
                                    <div className="flex justify-between text-[11px] font-black tracking-widest uppercase">
                                        ...
                                        <span className="text-slate-900">1 {selectedAsset.symbol} = {selectedAsset.price} {quoteAsset.symbol}</span>
                                    </div>
                                </div>
