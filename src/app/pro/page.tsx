            ...
            <div className="flex md:hidden flex-1 flex-col overflow-hidden min-h-0">
                ...
                <div className="flex-1 overflow-hidden relative">
                    <AnimatePresence mode="wait">
                        ...
                        {/* Pairs tab */}
                        {mobileTab === "pairs" && (
                            <motion.div key="pairs" className={`absolute inset-0 overflow-y-auto no-scrollbar ${PANEL_BG}`}
                                initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.2 }}>

                                <div className={`sticky top-0 z-20 flex flex-col bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b ${BORDER}`}>
                                    <div className={`p-3`}>
                                        <div className="relative">
                                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
                                            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
